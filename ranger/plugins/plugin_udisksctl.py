    ...
    try:
        ...
        for disk in "abcdefgh":
            ...
            for part in "123456789":
                fm.execute_console("map {key}{0}{1} chain shell udisksctl mount -b /dev/sd{0}{1}".format(disk, part, key=MOUNT_KEY))
                fm.execute_console("map {key}{0}{1} chain shell udisksctl unmount -b /dev/sd{0}{1}".format(disk, part, key=UMOUNT_KEY))
    finally:
